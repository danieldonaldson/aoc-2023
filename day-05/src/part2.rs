...
pub fn process(input: &str) -> i64 {
    ...
    for seed_range in seeds.iter() {
        (seed_range.start..=seed_range.end)
            .into_par_iter()
            .progress_count((seed_range.start..=seed_range.end).count() as u64)
            .for_each(|seed| {
                // println!(
                //     "checking seeds from {} to {}",
                //     seed_range.start, seed_range.end
                // );
...
fn parse_input(
    ...
    let mut seeds_iter = seeds_section.split(' ');

    let mut seeds = vec![];

    while let Some(seed) = seeds_iter.next() {
        let seed_start = seed.parse::<i64>().unwrap();
        let seed_range = seeds_iter.next().unwrap().parse::<i64>().unwrap();

        seeds.push(SeedRange {
            start: seed_start,
            end: seed_start + seed_range - 1,
        });
    }
